...
if(isset($_POST['submit'])){
    ...
    if($user_name ===""){
        ...
        if($is_admin ===""){
            $status=false;
            $is_admin_err="Select the user role!";
        }
        
        if($status){
            $status = save_user($mysqli,$user_name,$email,$date_of_birth,$phoneno,$sex,$class,$hash_password,$is_admin);
            if($status){
                $success=true;
                $invalid=false;
            }
...
<nav class="navbar navbar-expand-lg">
    ...
    <div class="row d-flex justify-content-center">
        <div class="card mb-3 crd" style="width: 700px;">
                <?php
                if($success) echo '<div class="alert alert-primary">Registration Success!
                <div>Please wait a second to get the unique code.</div>
                </div>';
                if($invalid) echo '<div class="alert alert-danger">Oops... Something went wrong! Please try again later.</div>';
                ?>
                    ...
                    <div class="col-">
                        <div class="p-4">
                            <div class="card-header mb-3 text-white crdhead" style="width: 100%;height:50px;border-radius:50px;">
                                <h4 class="text-center fw-bold">REGISTERATION</h4>
                            </div>
                            <form class="user" method="post">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control form-control-user" 
                                            placeholder="Enter Student Name" name="user_name"  value="<?php echo $user_name?>">
                                    ...
                                    <select id="is_admin" name="is_admin" class="form-control" style="border-radius: 50px;">
                                        <option class="form-control" disabled selected>Select</option>
                                        <option value="user" class="form-control form-control-user" <?php if ($is_admin == 'user') echo 'selected' ?>>User</option>
                                        <option value="admin" class="form-control form-control-user" <?php if ($is_admin == 'admin') echo 'selected' ?>>Admin</option>
                                    </select>
...
<div id="popup" class="popup">
    <div class="popup-content">
        <div class="">
        <p class="text-danger">This is the unique code for each user. Copy this code and save it up for later Sign in process!</p>
        <label>Code:</label>
        <p name="code"> <?php echo $password; ?></p>
        </div>
        <button type="submit" value="OK" name="ok" onclick="close()" class="btn btn-primary">OK</button>
    </div>
</div>
                                    </div>
                                    <div class="col-3 me-5">
                                    <button class="bbtt2 btn btn-user btn-block" name='reset' type='reset'>
                                    Cancel
                                    </button>
                                </div>
